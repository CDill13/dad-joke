...
describe('NoCacheHeadersInterceptor', () => {
  ...
  it('should add no-cache headers to HTTP requests', () => {
    ...
    interceptor
      .intercept(request, TestBed.inject(HttpHandler))
      .subscribe((event) => {
        if (event instanceof HttpResponse) {
          expect(event.headers.has('Cache-Control')).toBe(
            true,
          ...
          expect(event.headers.has('Pragma')).toBe(
            true,
            'Missing Pragma header'
          );
          expect(event.headers.get('Pragma')).toBe(
            'no-cache',
